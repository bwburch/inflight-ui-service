...
func (s *Store) List(ctx context.Context, role string, isActive *bool, limit, offset int) ([]User, int, error) {
	...
	for rows.Next() {
		var u User
		err := rows.Scan(&u.ID, &u.Username, &u.Email, &u.FullName, &u.Role, &u.IsActive,
...
func (s *Store) Create(ctx context.Context, input CreateUserInput) (*User, error) {
	...
	).Scan(&u.ID, &u.Username, &u.Email, &u.FullName, &u.Role, &u.IsActive,
		&u.CreatedAt, &u.UpdatedAt, &u.LastLoginAt)
...
func (s *Store) Update(ctx context.Context, id int, input UpdateUserInput) (*User, error) {
	...
	query += fmt.Sprintf(" WHERE id = $%d RETURNING id, username, email, full_name, role, is_active, created_at, updated_at, last_login_at", argCount)
	args = append(args, id)
